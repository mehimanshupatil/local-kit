...
import { DownloadSimpleIcon, ArchiveIcon, EyeIcon, EyeSlashIcon, CopyIcon, CheckIcon } from '@phosphor-icons/react';
  ...
  return (
    <div className="mt-2 space-y-2">
      <Button variant="secondary" size="sm" onClick={toggle}>
        {opened ? <EyeSlashIcon className="size-3.5" /> : <EyeIcon className="size-3.5" />}
...
function ImagePreview({ blob }: { blob: Blob }) {
  ...
  if (!url) return null;
  return (
    <div className="w-16 h-16 rounded-lg shrink-0 border border-border bg-secondary flex items-center justify-center overflow-hidden">
      <img
        src={url}
        alt="preview"
  ...
  return (
    <Card className="animate-slide-up">
      ...
      <CardContent className="space-y-2">
        {files.map((file, i) => (
          <div key={i} className="px-4 py-3 rounded-xl bg-secondary">
